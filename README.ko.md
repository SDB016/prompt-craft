...
**2단계: 셋업 (최초 1회)**
```
/review          # 첫 실행: 리뷰 저장소를 묻고 → 자동으로 모든 설정 완료
```

**3단계: 사용**
```
git push         # 프롬프트 자동 캡처 (셋업 후)
/review          # 프롬프트 리뷰 PR 생성
/score           # 로컬 스코어링 (PR 생성 없음)
```

첫 `/review`가 곧 셋업입니다 — 리뷰 저장소를 묻고 나머지는 자동으로 설정됩니다. 이후 매 `git push`마다 프롬프트가 자동 캡처되고, `gh pr create` 시 점수가 매겨집니다.
