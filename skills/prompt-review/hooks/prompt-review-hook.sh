...
#
# Protocol: exit 0 + JSON { decision: "block", reason: "..." } → prompts Claude
...
if echo "$COMMAND" | grep -qE '(^|&&\s*|\|\|\s*|;\s*)git\s+push\b' && \
  ...
  jq -n --arg reason '[PROMPT CAPTURE] git push detected.

You MUST use the Skill tool to run: /prompt-craft:prompt-review --push
...
if echo "$COMMAND" | grep -qE '(^|&&\s*|\|\|\s*|;\s*)gh\s+pr\s+create\b'; then
  ...
  jq -n --arg reason '[PROMPT REVIEW] gh pr create detected.

You MUST use the Skill tool to run: /prompt-craft:prompt-review
