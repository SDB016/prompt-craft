...
# Prompt Score — Local Prompt Quality Check
...
2. Score against the same 8 criteria used by `/review`
...
|---------|-------------|
| `/score` | Score current session, show scorecard + tips |
...
```
No prompts to score yet. Start working and run /score again later.
```
...
Score all collected prompts against 8 criteria. Same scoring system as `/review`.
...
```
Prompt Score — {PROMPT_COUNT} prompts scored
...
When ready for team review, use /review.
```
...
| Error | Message |
|-------|---------|
| No prompts found | `No prompts to score yet. Start working and run /score again later.` |
| Session too short | `Error: Only greetings found, no substantive prompts.` / `Cause: Session hasn't started real work yet.` / `Fix: Start working on a task and run /score again.` |
...
2. **Same scoring as `/review`.** Identical 8 criteria and weights. What you see here is what the PR will show.
