...
**ステップ2: セットアップ（初回のみ）**
```
/review          # 初回実行: レビューリポジトリを確認 → すべて自動設定
```

**ステップ3: 使用**
```
git push         # プロンプト自動キャプチャ（セットアップ後）
/review          # プロンプトレビューPR作成
/score           # ローカルスコアリング（PR作成なし）
```

最初の`/review`がセットアップを兼ねています — レビューリポジトリを確認し、残りはすべて自動設定されます。以降、`git push`のたびにプロンプトが自動キャプチャされ、`gh pr create`時にスコアリングされます。

> **注意:** 初回インストール後、push hookを有効にするにはClaude Codeを再起動してください。
